...
class TermControllerTest {
    ...
    void create_admin_returns201() throws Exception {
        ...
        when(termService.create(any(MasterDataRequest.class), eq(1L)))
        ...
        mockMvc.perform(post("/api/admin/terms")
                ...
                .andExpect(jsonPath("$.data.code").value("TRM-001"))
                .andExpect(jsonPath("$.data.name").value("Fall 2026"));
    ...
    void getAll_admin_returns200WithPage() throws Exception {
        ...
        when(termService.getAll(any()))
                .thenReturn(new PageImpl<>(List.of(sampleResponse()), PageRequest.of(0, 25), 1));

        mockMvc.perform(get("/api/admin/terms"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.content[0].code").value("TRM-001"));
    ...
    void getById_admin_returns200() throws Exception {
        ...
        when(termService.getById(1L)).thenReturn(sampleResponse());

        mockMvc.perform(get("/api/admin/terms/1"))
                .andExpect(status().isOk())
    ...
    void getById_notFound_returns404() throws Exception {
        ...
        when(termService.getById(99L))
                .thenThrow(new ResourceNotFoundException("Term not found: 99"));

        mockMvc.perform(get("/api/admin/terms/99"))
    ...
    void update_admin_returns200() throws Exception {
        ...
        updated.setName("Updated Term");

        when(termService.update(eq(1L), any(MasterDataRequest.class), eq(1L)))
                .thenReturn(updated);
        ...
        mockMvc.perform(put("/api/admin/terms/1")
                        ...
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.name").value("Updated Term"));
    ...
    void update_duplicateCode_returns400() throws Exception {
        ...
        when(termService.update(eq(1L), any(MasterDataRequest.class), eq(1L)))
                .thenThrow(new BusinessException("Code already exists: TRM-DUP"));

        mockMvc.perform(put("/api/admin/terms/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Code already exists: TRM-DUP"));
    ...
    void delete_admin_returns200() throws Exception {
        ...
        doNothing().when(termService).delete(eq(1L), eq(1L));
    ...
    void delete_referentialIntegrityViolation_returns400() throws Exception {
        ...
        doThrow(new BusinessException("Cannot delete: term is in use"))
                .when(termService).delete(eq(1L), eq(1L));

        mockMvc.perform(delete("/api/admin/terms/1"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Cannot delete: term is in use"));
