...
class CourseControllerTest {
    ...
    void create_admin_returns201() throws Exception {
        ...
        when(courseService.create(any(MasterDataRequest.class), eq(1L)))
        ...
        mockMvc.perform(post("/api/admin/courses")
                ...
                .andExpect(jsonPath("$.data.code").value("CRS-001"))
    ...
    void getAll_admin_returns200WithPage() throws Exception {
        ...
        when(courseService.getAll(any()))
                .thenReturn(new PageImpl<>(List.of(sampleResponse()), PageRequest.of(0, 25), 1));

        mockMvc.perform(get("/api/admin/courses"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.content[0].code").value("CRS-001"));
    ...
    void getById_admin_returns200() throws Exception {
        ...
        when(courseService.getById(1L)).thenReturn(sampleResponse());
    ...
    void getById_notFound_returns404() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/courses/99"))
    ...
    void update_admin_returns200() throws Exception {
        ...
        when(courseService.update(eq(1L), any(MasterDataRequest.class), eq(1L)))
                .thenReturn(updated);
    ...
    void update_duplicateCode_returns400() throws Exception {
        ...
        when(courseService.update(eq(1L), any(MasterDataRequest.class), eq(1L)))
                .thenThrow(new BusinessException("Code already exists: CRS-DUP"));

        mockMvc.perform(put("/api/admin/courses/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Code already exists: CRS-DUP"));
    ...
    void delete_admin_returns200() throws Exception {
        ...
        doNothing().when(courseService).delete(eq(1L), eq(1L));
    ...
    void delete_referentialIntegrityViolation_returns400() throws Exception {
        ...
        doThrow(new BusinessException("Cannot delete: course is in use"))
                .when(courseService).delete(eq(1L), eq(1L));

        mockMvc.perform(delete("/api/admin/courses/1"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Cannot delete: course is in use"));
